...
class post_train:
    def __init__(self, env_handler: EnvHandler, num_episodes=100, max_ep_len=150, acceler_vel = 1.0, reset_mode=3, render=True, save=True, neighbor_region=4, neighbor_num=5, args=None, **kwargs):
    ...
    def policy_test(self, policy_type='drl', policy_path=None, policy_name='policy', result_path=None, result_name='/result.txt', save_anim_name=None, figure_save_path=None, ani_save_path=None, policy_dict=False, once=False):
        ...
        recieved_goal_rewards = [False] * self.robot_number
        figure_id = 0

        save_anim = self.render or self.save
        ...
        while n < self.num_episodes:
            ...
            if policy_type == 'drl': 
                ...
                for i in range(self.robot_number):
                    ...
                    cur_vel = self.env.robot_list[i].velocity
                    abs_action = a_inc + np.squeeze(cur_vel)
                    abs_action_list.append(abs_action)

            o = self.env.step(abs_action_list, vel_type = 'omni')
            r = self.env.calculate_dense_reward(o, recieved_goal_rewards)

            robot_speed_list = [np.linalg.norm(robot.velocity) for robot in self.env.robot_list]
            ...
            if any([robot.collision for robot in self.env.robot_list]) or (ep_len == self.max_ep_len) or all(recieved_goal_rewards):
                # fix rendering
                speed = np.mean(speed_list)
                figure_id = 0
                if all(recieved_goal_rewards):
        ...
        if save_anim:
            self.env_handler.end_save_ani_env(save_anim_name)
    ...
    def load_policy(self, filename, std_factor=1, policy_dict=False):
        ...
        if policy_dict == True:
            ...
            model = torch.load(filename, map_location=self.args.device)
            model.eval()
